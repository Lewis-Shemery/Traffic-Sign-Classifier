...
class datasetGenerator:
	...
	def __init__(self, nClasses=None, nTrainSamples=None, 
		...
		self.destPath = './data'
		self.signNameFile = './signnames.csv'
		...
		if self.nTrainSamples is not None:
			self.dataLen = nTrainSamples

		if self.nTestSamples is not None:
			self.dataLen += nTestSamples

		if self.nValidateSamples is not None:
			self.dataLen += nValidateSamples

	def readSignNameCSV(self):
		with open(self.signNameFile, 'r') as fp:
			data = list(csv.DictReader(fp))

		data = [(self.dirNameFormatter(everyData['ClassId']), 
				 everyData['SignName'])
	...
	def readCSV(self, csvPath):
		...
		data = [self.imageFileFormatter(everyData[0].split(';')) 
				for everyData in data]
	...
	def createDataSet(self):
		...
		if self.nTrainSamples is not None:
			...
			offset = 0
			self.Xtrain = [image[0] for image in images[offset:offset+self.nTrainSamples]]

			self.Ytrain = [image[1] for image in images[offset:offset+self.nTrainSamples]]

			if self.nTestSamples is not None:
				print ("Creating test samples...")
				offset += self.nTrainSamples
				self.Xtest = [image[0] for image in images[offset:offset+self.nTestSamples]]

				self.Ytest = [image[1] for image in images[offset:offset+self.nTestSamples]]
			...
			if self.nValidateSamples is not None:
				...
				self.Xvalidate = [image[0] for image in images[offset+1:offset+self.nValidateSamples]]
				self.Yvalidate = [image[1] for image in images[offset+1:offset+self.nValidateSamples]]
	...
	def generateInfo(self):
		...
		self.info += "nValidateSamples = {}".format(self.nValidateSamples)+nl+nl
		...
		self.info += "shape of Xvalidate = {}".format(np.shape(self.Xvalidate))+nl
		self.info += "shape of Yvalidtae = {}".format(np.shape(self.Yvalidate))+nl+nl
